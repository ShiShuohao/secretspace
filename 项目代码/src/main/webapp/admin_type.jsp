...
<head>
    ...
    <style>
        #input_submit:hover,#delete:hover{
...
<div style="width: 100%;height: 90px;position: fixed;top: 0;left: 0;background: white;box-shadow: 0 0 3px 3px #f3f3f3;z-index: 1;">
    ...
    <a href="admin_type.jsp" id="a10" style="position: absolute;font-size: 16px;top: 75%;left: 61%;transform: translate(-50%,-50%);text-decoration: none;color: black;transition: 0.5s;font-weight: bold">板块管理</a>
    ...
    <form action="admin_type_insert" method="post" id="form" style="position: absolute;height: 100px;width: 50%;left: 25%;margin-top: 88px;box-shadow: 0 0 3px 3px #f3f3f3;background: white;">
        <span style="position: absolute;top: 50%;left: 31%;transform: translate(-50%,-50%);font-size: 17px;">新增板块</span>
        <input type="text" name="type" id="type" placeholder="请输入板块名称" style="position: absolute;top: 50%;left: 51%;transform: translate(-50%,-50%);height: 25px;width: 25%;">
        <span id="error" style="position: absolute;color: red;transform: translate(-50%,-50%);font-size: 12px;top: 75%;left: 70%;"></span>
        <input type="button" id="input_submit" value="提&nbsp;&nbsp;&nbsp;交" style="position: absolute;top: 50%;left: 70%;transform: translate(-50%,-50%);height: 28px;width: 58px;border-radius: 3px;border: 0;background: #0095ff;color: white;transition: 0.5s;">
    </form>
    <%
        List<Type> listStatic = null;
        try {
            listStatic = TypeDao.selectTypeByState(0);
        } catch (Exception e) {
            e.printStackTrace();
        }
        List<Type> listVariable = null;
        try {
            listVariable = TypeDao.selectTypeByState(1);
        } catch (Exception e) {
            e.printStackTrace();
        }
        request.setAttribute("listStatic",listStatic);
        request.setAttribute("listVariable",listVariable);
    %>
<div style="position: absolute;height: auto;width: 50%;left: 25%;background: white;box-shadow: 0 0 3px 3px #f3f3f3;text-align: center;margin-top: 193px;">
    <c:forEach var="listStatic" items="${listStatic}">
        <div style="height: 30px;border-bottom: 1px solid #f6f6f6;transition: 0.5s;" class="div_type">
            <span style="position: relative;top: 5px;left: -100px;">${listStatic.name}</span>
        </div>
    </c:forEach>
    <c:forEach var="listVariable" items="${listVariable}">
        <div style="height: 30px;border-bottom: 1px solid #f6f6f6;transition: 0.5s;" class="div_type">
            <span style="position: relative;top: 5px;left: -76.5px;">${listVariable.name}</span>
            <a href="admin_type_delete?type=${listVariable.name}" id="delete" style="text-decoration: none;position: relative;top: 3.5px;font-size: 13px;background: #0095ff;color: white;padding-right: 5px;padding-left: 5px;padding-top: 3px;padding-bottom: 3px;border-radius: 5px;transition: 0.5s;left: 90px;">删&nbsp;除</a>
        </div>
    </c:forEach>
</div>
    <script>
