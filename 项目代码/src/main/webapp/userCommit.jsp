...
<%@ page import="com.pojo.Commit_Blog" %>
<%@ page import="com.dao.Commit_BlogDao" %>
...
<div style="width: 100%;height: 90px;position: fixed;top: 0;left: 0;background: white;box-shadow: 0 0 3px 3px #f3f3f3;z-index: 1;">
    ...
    <a href="userCommit.jsp" id="a6" style="position: absolute;font-size: 16px;top: 75%;left: 34%;transform: translate(-50%,-50%);text-decoration: none;color: black;transition: 0.5s;font-weight: bold;">我的回复</a>
...
<div style="position: absolute;height: auto;width: 70%;left: 13.8%;background: white;box-shadow: 0 0 3px 3px #f3f3f3;text-align: center;margin-top: 88px;padding-left: 1.2%;padding-right: 1.2%;">
    <%
        ...
        List<Commit_Blog> list = null;
        try {
            list = Commit_BlogDao.selectCommit_Blog(username);
        } catch (Exception e) {
            e.printStackTrace();
        }
        request.setAttribute("commit_blogList",list);
    %>
    <table style="width: 100%;line-height: 50px;">
        <c:forEach var="commit_blog" items="${commit_blogList}">
            <tr>
                <td><a href="blog_content?id=${commit_blog.blog_id}" id="content" style="text-decoration: none;font-size: 18px;color: black;">[${commit_blog.type}]&nbsp;${commit_blog.headline}</a></td>
                <td style="width: 15%;">${commit_blog.commit_uptime}</td>
                <td style="width: 10%;"><a href="commit_delete?id=${commit_blog.id}" style="text-decoration: none;font-size: 13px;background: #0095ff;color: white;padding-right: 5px;padding-left: 5px;padding-top: 3px;padding-bottom: 3px;border-radius: 5px;transition: 0.5s;" id="delete">删除评论</a></td>
            </tr>
        </c:forEach>
    </table>
</div>
